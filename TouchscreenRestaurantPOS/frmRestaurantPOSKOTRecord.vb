...
Public Class frmRestaurantPOSKOTRecord
    ...
    Public Sub GetData()
        Try
            ...
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_OrderInfoKOT.Id), RTRIM(TicketNo),BillDate,RTRIM(TableNo),(RestaurantPOS_OrderInfoKOT.GrandTotal),RTRIM(Operator),RTRIM(GroupName),RTRIM(TicketNote) from RestaurantPOS_OrderInfoKOT  order by BillDate"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_OrderInfoKOT.Id), RTRIM(TicketNo),BillDate,RTRIM(TableNo),(RestaurantPOS_OrderInfoKOT.GrandTotal),RTRIM(Operator),RTRIM(GroupName),RTRIM(TicketNote) from RestaurantPOS_OrderInfoKOT where BillDate >=@d1 and BillDate < @d2 order by BillDate"
            cmd = New SqlCommand(sql, con)
    ...
    Private Sub cmbTicketNo_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbTicketNo.SelectedIndexChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_OrderInfoKOT.Id), RTRIM(TicketNo),BillDate,RTRIM(TableNo),(RestaurantPOS_OrderInfoKOT.GrandTotal),RTRIM(Operator),RTRIM(GroupName),RTRIM(TicketNote) from RestaurantPOS_OrderInfoKOT where TicketNo=@d1 order by BillDate"
            cmd = New SqlCommand(sql, con)
