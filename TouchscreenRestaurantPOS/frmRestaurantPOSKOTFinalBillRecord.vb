...
Public Class frmRestaurantPOSKOTFinalBillRecord
    ...
    Public Sub GetData()
        Try
            ...
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoKOT.Id), RTRIM(BillNo),BillDate,KOTDiscountPer,RTRIM(DiscountReason),(RestaurantPOS_BillingInfoKOT.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate from RestaurantPOS_BillingInfoKOT  order by BillDate"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoKOT.Id), RTRIM(BillNo),BillDate,KOTDiscountPer,RTRIM(DiscountReason),(RestaurantPOS_BillingInfoKOT.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate from RestaurantPOS_BillingInfoKOT where  BillDate >=@d1 and BillDate < @d2 order by BillDate"
            cmd = New SqlCommand(sql, con)
    ...
    Private Sub cmbBillNo_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbBillNo.SelectedIndexChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoKOT.Id), RTRIM(BillNo),BillDate,KOTDiscountPer,RTRIM(DiscountReason),(RestaurantPOS_BillingInfoKOT.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate from RestaurantPOS_BillingInfoKOT where BillNo=@d1 order by BillDate"
            cmd = New SqlCommand(sql, con)
