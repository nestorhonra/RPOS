...
Public Class frmRestaurantPOSTARecord
    ...
    Public Sub GetData()
        Try
            ...
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoTA.Id), RTRIM(BillNo),BillDate,TADiscountPer,RTRIM(DiscountReason),SubTotal,ParcelCharges,(RestaurantPOS_BillingInfoTA.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate from RestaurantPOS_BillingInfoTA  order by BillDate"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoTA.Id), RTRIM(BillNo),BillDate,TADiscountPer,RTRIM(DiscountReason),SubTotal,ParcelCharges,(RestaurantPOS_BillingInfoTA.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate from RestaurantPOS_BillingInfoTA where  BillDate >=@d1 and BillDate < @d2 order by BillDate"
            cmd = New SqlCommand(sql, con)
    ...
    Private Sub cmbBillNo_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbBillNo.SelectedIndexChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoTA.Id), RTRIM(BillNo),BillDate,TADiscountPer,RTRIM(DiscountReason),SubTotal,ParcelCharges,(RestaurantPOS_BillingInfoTA.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate from RestaurantPOS_BillingInfoTA where BillNo=@d1 order by BillDate"
            cmd = New SqlCommand(sql, con)
