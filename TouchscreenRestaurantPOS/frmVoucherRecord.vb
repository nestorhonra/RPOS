...
Public Class frmVoucherRecord
    ...
    Public Sub GetData()
        Try
            ...
            con.Open()
            cmd = New SqlCommand("Select RTRIM(Voucher.Id) as [Voucher ID], RTRIM(VoucherNo) as [Voucher No.],Convert(DateTime,Date,103) as [Voucher Date], RTRIM(Name) as [Name],RTRIM(Details) as [Details],RTRIM(PaymentMode) as [Payment Mode],RTRIM(Voucher.GrandTotal) as [Grand Total] from Voucher order by Date", con)
    ...
    Private Sub dgw_MouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles dgw.MouseClick
        Try
            ...
            frmVoucher.dtpDate.Value = dr.Cells(2).Value.ToString()
            frmVoucher.txtName.Text = dr.Cells(3).Value.ToString()
            frmVoucher.txtDetails.Text = dr.Cells(4).Value.ToString()
            frmVoucher.cmbPaymentMode.Text = dr.Cells(5).Value.ToString()
            frmVoucher.txtGrandTotal.Text = dr.Cells(6).Value.ToString()
            ...
            frmVoucher.cmbPaymentMode.Enabled = False
            con = New SqlConnection(cs)
            con.Open()
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("Select RTRIM(Voucher.Id) as [Voucher ID], RTRIM(VoucherNo) as [Voucher No.],Convert(DateTime,Date,103) as [Voucher Date], RTRIM(Name) as [Name],RTRIM(Details) as [Details],RTRIM(PaymentMode) as [Payment Mode],RTRIM(Voucher.GrandTotal) as [Grand Total] from Voucher where Date between @d1 and @d2 order by Date", con)
    ...
    Private Sub cmbBillNo_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbVoucherNo.SelectedIndexChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("Select RTRIM(Voucher.Id) as [Voucher ID], RTRIM(VoucherNo) as [Voucher No.],Convert(DateTime,Date,103) as [Voucher Date], RTRIM(Name) as [Name],RTRIM(Details) as [Details],RTRIM(PaymentMode) as [Payment Mode],RTRIM(Voucher.GrandTotal) as [Grand Total] from Voucher where VoucherNo='" & cmbVoucherNo.Text & "' order by Date", con)
            Dim myDA As SqlDataAdapter = New SqlDataAdapter(cmd)
            Dim myDataSet As DataSet = New DataSet()
            myDA.Fill(myDataSet, "Voucher")
            dgw.DataSource = myDataSet.Tables("Voucher").DefaultView
